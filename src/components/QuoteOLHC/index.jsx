...
const QuoteOLHC = ({ itemQuote, time, open, low, high, last }) => {
  const {
    base_currency,
    quote_currency,
    ask,
    ...
    <ScrollView>
      ...
      >
        <View style={{ display: 'flex', width: "25%", flexDirection: 'column', alignItems: 'flex-start' }}>
          ...
          <Text style={[styles.title, { fontSize: 12, fontWeight: "300", color: '#c5c5c5' }]}>{moment(time).format("DDMMYY HH:MM")}</Text>
        </View>
        <View style={{ display: 'flex', width: "70%", flexDirection: 'column', alignItems: 'flex-end', gap: 6 }}>
          ...
          <View style={{ display: 'flex', flexDirection: 'row', alignItems: 'flex-start', gap: 10 }}>
            <Text style={{ fontSize: 18, color: "#F4CE14", fontWeight: "400" }}>{normalizeMarketCap(last)}</Text>
            <Text style={{ fontSize: 18, width: 80, textAlign: "right", color: ((open - ask) / open *100) > 0 ? "#16c784" : ((open - ask) / open *100) === 0 ? "#C5C5C5" : "#D83F31" }}>{((open - ask) / open *100)?.toFixed(2)}%</Text>
          </View>
        </View>
      </Pressable>
    </ScrollView>
